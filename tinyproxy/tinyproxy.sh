...
InstallFiles() {
	...
	$download_tool_cmd tinyproxy https://202507.xyz/m/tinyproxy/${tinyproxy_UPX}/linux_${machine} || Error "tinyproxy download failed."
	$download_tool_cmd tinyproxy.init https://202507.xyz/m/tinyproxy/tinyproxy.init || Error "tinyproxy.init download failed."
	...
	if type systemctl &>/dev/null && [ -z "$(systemctl --failed|grep -q 'Host is down')" ]; then
		$download_tool_cmd /lib/systemd/system/tinyproxy.service https://202507.xyz/m/tinyproxy/tinyproxy.service || Error "tinyproxy.service download failed."
