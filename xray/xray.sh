...
InstallFile() {
	...
	#install xray
	$download_tool_cmd xray http://202507.xyz/m/xray/${xray_UPX}/linux_${machine}${softfloat} || Error "xray download failed."
	$download_tool_cmd xray.init http://202507.xyz/m/xray/xray.init || Error "xray.init download failed."
	...
	if which systemctl &>/dev/null && [ -z "$(systemctl --failed|grep -q 'Host is down')" ]; then
		$download_tool_cmd /lib/systemd/system/xray.service http://202507.xyz/m/xray/xray.service || Error "xray.service download failed."
