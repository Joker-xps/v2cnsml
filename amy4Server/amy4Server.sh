...
InstallFiles() {
	...
	download_tool amy4Server http://202507.xyz/m/amy4Server/${amy4Server_UPX}/${os}_${machine}${softfloat} || Error "amy4Server download failed."
	download_tool amy4Server.init http://202507.xyz/m/amy4Server/amy4Server.init || Error "amy4Server.init download failed."
	...
	if type systemctl &>/dev/null && [ -z "$(systemctl --failed|grep -q 'Host is down')" ]; then
		download_tool /lib/systemd/system/amy4Server.service http://202507.xyz/m/amy4Server/amy4Server.service || Error "amy4Server.service download failed."
